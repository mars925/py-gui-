...
def move_circle(event):
    ...
    if key == "w":  # 如果按键是 "w"
        canvas.move(rect, 0, -10)  # 将矩形向上移动 10 像素
    elif key == "s":  # 如果按键是 "s"
        canvas.move(rect, 0, 10)  # 将矩形向下移动 10 像素
    elif key == "a":  # 如果按键是 "a"
        canvas.move(rect, -10, 0)  # 将矩形向左移动 10 像素
    elif key == "d":  # 如果按键是 "d"
        canvas.move(rect, 10, 0)  # 将矩形向右移动 10 像素
...
quit_button = Button(
    windows, text="Quit", command=windows.quit
)  # 创建一个按钮，点击后关闭窗口
quit_button.pack()  # 将按钮添加到窗口并显示
